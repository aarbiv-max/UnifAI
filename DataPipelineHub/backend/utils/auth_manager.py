...
class AuthManager:
    ...
    def init_app(self, app):
        ...
        # Configure OAuth (skip in DEV_MODE to avoid discovery/redirect loops during local dev)
        if not dev_mode:
        ...
        app.config.update({
            'SESSION_COOKIE_SECURE': (self.backend_env == 'production'),
            'SESSION_COOKIE_HTTPONLY': True,
            # In production, cross-origin cookies require SameSite=None; in dev avoid forcing None over HTTP
            'SESSION_COOKIE_SAMESITE': 'None' if self.backend_env == 'production' else 'Lax',
    ...
    def _register_auth_routes(self):
        ...
        def login():
            ...
            if dev_mode:
                # :point_down: Bypass SSO in development mode
                session['user'] = {
                    'username': 'dev_user',
                ...
                # Mirror real SSO shape: set top-level token expiry to avoid refresh loop in dev
                session['token_expires_at'] = session['user']['token_expires_at']
        ...
        def auth_callback():
            ...
            if dev_mode:
                return redirect(f"{config.frontend_url}?auth=success")
            # :point_down: Keep the existing code for real SSO users
            try:
    ...
    def _should_refresh_token(self):
        ...
        if dev_mode:
            return False
        token_expires_at = session.get('token_expires_at') or session.get('user', {}).get('token_expires_at', 0)
    ...
    def _refresh_access_token(self):
        ...
        if self.keycloak_client is None:
            logger.info("Skipping token refresh: Keycloak client not initialized (DEV_MODE)")
            return False
