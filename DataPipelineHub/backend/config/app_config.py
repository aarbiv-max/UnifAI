...
class AppConfig(SharedConfig):
    ...
    # broker options
    broker_heartbeat: int = 60  # Heartbeat interval in seconds between worker and RabbitMQ
    broker_tcp_user_timeout: int = 600000  # TCP timeout in ms (10 minutes)
    broker_socket_keepalive: bool = True  # Enable TCP keepalive to maintain idle connections

    # Celery task handling options
    task_acks_late: bool = True  # Acknowledge tasks only after completion (prevents loss)
    task_reject_on_worker_lost: bool = True  # Requeue task if worker dies
    worker_cancel_long_running_tasks_on_connection_loss: bool = False  # Continue long tasks even if broker disconnects
