...
class CypressCodeAnalyzer(BaseAnalyzer):
    ...
    def match_cy_command(
        ...
        for repo_cmd in repo_commands:
            if cmd["command"] == repo_cmd["command"]:
                return (
                    ...
                    repo_cmd,
                    "exact",
                )  # This matches the custom command defined in Cypress.Commands.add
        ...
        for canonical, wrappers in self.detected_cy_wrappers.items():
            if cmd["command"] in wrappers:
                for repo_cmd in repo_commands:
                    if (
                        repo_cmd["command"] == canonical
                        or repo_cmd["command"] in wrappers
                    ):
    ...
    def verify_code_snippet(self, code: str):
        ...
        verification = {"testBlocks": [], "cyCommands": []}

        # Test block verification

        repo_test_blocks = {
            (tb["type"], tb["name"]): tb for tb in self.symbol_database["testBlocks"]
        }
            ...
            verification["testBlocks"].append(
                {
                    ...
                    "url":    self.gitRepoLink + match["file"] if match else None,
                    "issues": issues,
                }
            )
        ...
        for cmd in snippet_data.get("cyCommands", []):
            matched, match, match_level = self.match_cy_command(
                cmd, self.symbol_database["cyCommands"]
            )
            verification["cyCommands"].append(
                {
                    ...
                    "url":     (
                                self.gitRepoLink + match["file"]
                                if matched and match.get("file")
                                else None
                               ),
