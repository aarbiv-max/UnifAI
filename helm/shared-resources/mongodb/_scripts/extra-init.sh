...
main() {
    ...
    if [ "$REPLICASET_ENABLED" = "true" ]; then
        wait_ready "$RETRIES" "$DELAY" "$INIT_DELAY"

        if [ "$MONGOTYPE" = "ARBITER" ]; then
            init_arbiter "$HEADLESS_SVC" "$ARBITER_SVC"
        elif [ "$MONGOTYPE" = "HIDDEN" ]; then
            init_hidden "$HEADLESS_SVC" "$HIDDEN_SVC"
        else
            init_replicaset "$HEADLESS_SVC" "$REPLICASET_NAME"
        fi

        terminatedelay="$TERMINATE_DELAY"
    fi
