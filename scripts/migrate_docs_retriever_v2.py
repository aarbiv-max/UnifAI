#!/usr/bin/env python3
"""
Migrate 'docs_dataflow' retrievers to 'docs_rag' format.

Usage:
    # Dry run on resources collection (default query)
    python migrate_docs_retriever_v2.py
    
    # Dry run with custom query
    python migrate_docs_retriever_v2.py --query '{"category": "retrievers", "type": "docs_dataflow"}'
    
    # Specify collection
    python migrate_docs_retriever_v2.py --collection resources --query '{"type": "docs_dataflow"}'
    
    # Apply changes
    python migrate_docs_retriever_v2.py --apply
"""
...
# Default query for docs_dataflow retrievers
DEFAULT_QUERY = {"category": "retrievers", "type": "docs_dataflow"}
...
def transform_cfg_dict(old_cfg: dict) -> dict:
    """
    Transform old 'docs_dataflow' cfg_dict to new 'docs_rag' format.
    ...
    Note: Only the type field changes. All other fields are preserved.
    """
    new_cfg = old_cfg.copy()
    new_cfg["type"] = "docs_rag"
    return new_cfg
...
def main():
    parser = argparse.ArgumentParser(
        description="Migrate 'docs_dataflow' retrievers to 'docs_rag' format"
    )
    parser.add_argument(
        "--collection", "-c",
