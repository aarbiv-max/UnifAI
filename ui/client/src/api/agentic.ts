...
// Internal blueprint names that should be hidden from users
const INTERNAL_BLUEPRINT_NAMES = ['Workflow Builder'];

// Fetch available blueprints (filters out internal system blueprints)
export async function fetchBlueprints(userId?: string): Promise<WorkflowBlueprint[]> {
  ...
  // Filter out internal system blueprints
  return blueprints.filter((bp: WorkflowBlueprint) => 
    !INTERNAL_BLUEPRINT_NAMES.includes(bp.name || bp.spec_dict?.name || '')
  );
}
