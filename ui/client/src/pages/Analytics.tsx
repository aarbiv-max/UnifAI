...
function ActiveTodayTable({ users, page, setPage, itemsPerPage, timeRange = 'today' }: any) {
  const getTitle = () => {
    switch (timeRange) {
      case 'today':
        return 'Active Today';
...
function BlueprintsTable({ blueprints, colors }: any) {
  const colorPalette = useMemo(() => {
    return generateColorPalette(colors.primary, blueprints?.length || 0);
  }, [colors.primary, blueprints?.length]);
