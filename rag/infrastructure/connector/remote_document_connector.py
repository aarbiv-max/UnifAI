...
This module provides a document connector that uses the external docling service
instead of processing documents locally.
"""
...
from infrastructure.clients.docling_service_client import (
    DoclingServiceClient,
    DoclingProcessingError,
)


class RemoteDocumentConnector(DataConnector):
    """
    Document connector for processing PDF and other document formats via remote service.
    ...
    def __init__(
        ...
        """
        Initialize the remote document connector.
        
        Args:
            config_manager: Configuration manager for document processing
            service_url: URL for the docling service
        ...
        if service_url is None:
            raise ValueError("service_url is required for RemoteDocumentConnector")
        
        config_timeout = timeout or self._config_manager.get_config_value("timeout_seconds")
        image_export_mode = self._config_manager.get_config_value("image_export_mode")
        
        self._service_client = DoclingServiceClient(
        ...
        logger.info("RemoteDocumentConnector initialized with docling service client")
    ...
    def authenticate(self) -> bool:
        ...
        """
        logger.info("Remote document connector does not require authentication")
        return True
    
    def test_connection(self) -> bool:
        """
        Test if the docling service is available.
    ...
    def process_document(self, document_path: str, upload_by: str = "default") -> Optional[Dict[str, Any]]:
        """
        Process a document file and extract text and metadata via remote service.
        ...
        try:
            logger.info(f"Processing document via remote service: {document_path}")
            ...
            logger.info(f"Document processed successfully via remote service: {document_path}")
    ...
    def process_documents(self, document_paths: List[str]) -> List[Dict[str, Any]]:
        ...
        """
        logger.info(f"Processing batch of {len(document_paths)} documents via remote service")
        results = []
        failed_count = 0
    ...
    def process_document_url(self, document_url: str, upload_by: str = "default") -> Optional[Dict[str, Any]]:
        """
        Process a document from a URL via remote service.
        ...
        """
        try:
            logger.info(f"Processing document from URL via remote service: {document_url}")
            ...
            logger.info(f"Document from URL processed successfully via remote service: {document_url}")
            return document_data
